  ...
  for (const event of events) {
    ...
    try {
      if (event.message.type === 'text') {
        ...
        // Fact correction command
        const trimmed = userMessage.trim();
        if (trimmed.toUpperCase().startsWith('FACT:') || trimmed.startsWith('ข้อเท็จจริง:')) {
          const fact = trimmed.replace(/^FACT:/i, '').replace(/^ข้อเท็จจริง:/, '').trim();
          const { saveFactOverride } = await import('@/lib/memory');
          await saveFactOverride(fact);
          await replyMessage(replyToken, `✅ Locked in:\n"${fact}"\n\nI'll treat this as authoritative from now on.`);
          continue;
        }
      ...
      try {
        ...
        const { buildMemoryContext, getFactOverrides } = await import('@/lib/memory');
        const { LOCKED_FACTS } = await import('@/lib/facts');
        ...
        const response = await client.messages.create({
          ...
          max_tokens: 1024,
          system: `You are Nai's AI chief of staff. Analyze this image in the context of his businesses: Planet Furniture factory, MuCat cat hotel, or stone sink products. Be direct and specific.\n\n${factsBlock}${memoryContext ? '\n\nContext: ' + memoryContext : ''}`,
          messages: [{
            role: 'user',
            content: [
